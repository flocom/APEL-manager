...
/**
 * Chaque réponse publique fait partir un e-mail vers l'adresse saisie
 * (confirmation, ou demande de confirmation d'un changement). Au-delà de
 * quelques-uns dans la journée vers la même boîte, la réponse est traitée mais
 * plus rien ne part : la personne a déjà reçu les précédents, et l'adresse
 * saisie n'est peut-être pas la sienne. Sans rien dire — le refus
 * apprendrait que l'adresse a servi.
 */
async function envoiAutorise(email: string): Promise<boolean> {
...
export async function envoyerConfirmationPresence({
  ...
  if (!baseUrl) return;
  if (!(await envoiAutorise(email))) return;
...
export async function avertirLeBureauPresence({
  ...
  try {
    ...
    if (!parti) {
      // Ni nom ni adresse dans le journal : l'identifiant de la réunion
      // suffit à retrouver la réponse dans l'onglet Présences.
      console.warn(
        `[presences] avis au bureau non remis (réunion ${reunion.id}).`,
      );
    }
  } catch (erreur) {
...
export async function demanderConfirmationDuChangement({
  ...
  // Le lien de confirmation porte un jeton scellé : même règle que le lien
  // de retrait, et même plafond par adresse.
  const baseUrl = secureLinkBaseUrl("Confirmation d'un changement de présence");
  if (!baseUrl) return;
  if (!(await envoiAutorise(email))) return;
  const association = await getAssociationSettings();
